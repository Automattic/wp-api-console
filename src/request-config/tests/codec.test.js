import { createRequestConfig, formatRequestConfig, parseRequestConfig } from '../codec';
...
it( 'preserves fixed contract field order when formatting a configuration', () => {
	const shuffled = {
		request: {
	...
	expect( formatRequestConfig( shuffled ) ).toBe( formatRequestConfig( expectedConfig ) );
} );
...
] )( 'rejects non-JSON %s values', ( _label, metadata ) => {
	expect( () =>
		createRequestConfig( {
			...source,
